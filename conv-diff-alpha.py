...
prev_H1_error = 0 
N = 32
for mu_value in [0.3**p for p in [4, 3, 2, 1, 0]]:
  ...
  # notice that when computing the rate vrt mu we should not use log! 

  L2_error = errornorm(U, u_analytical, mesh = mesh) 
  print ((" N = %d, L2 error = %e " % (N, L2_error)))
  if prev_L2_error > 0: 
    print ("rate of L2 error ", prev_L2_error / L2_error) 
  prev_L2_error = L2_error 

  H1_error = errornorm(U, u_analytical, "H1", mesh = mesh) 
